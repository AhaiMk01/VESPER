  ...
  for (int x = 0; x < M->xdim; x++)
  {
    ...
    for (int y = 0; y < M->ydim; y++)
    {
      for (int z = 0; z < M->zdim; z++)
      {
        ...
        if (dvec == 0.00)
          dvec = 1.00; // For low resolution maps!!
...
void ShowPath2(MRC *m, POINTS *p, GRAPH *g, TREE *t, int n)
{
  ...
  for (int mid = 0; mid < n; mid++)
  {
    ...
    for (int j = 0; j < p->Nori; j++)
    {
      ...
      {
        i = t[mid].Path[rnum];
        d2 = (p->cd[i][0] - tmp[0]) * (p->cd[i][0] - tmp[0]) + (p->cd[i][1] - tmp[1]) * (p->cd[i][1] - tmp[1]) + (p->cd[i][2] - tmp[2]) * (p->cd[i][2] - tmp[2]);
        if (d2 < mindis)
        {
          minid = rnum;
          mindis = d2;
  ...
  for (int x = 0; x < m->xdim; x++)
  {
    for (int y = 0; y < m->ydim; y++)
    {
      for (int z = 0; z < m->zdim; z++)
      {
        ...
        d2 = ((double)x - cent[0]) * ((double)x - cent[0]) + ((double)y - cent[1]) * ((double)y - cent[1]) + ((double)z - cent[2]) * ((double)z - cent[2]);
        if (d2 > dmax)
          dmax = d2;
      }
    }
